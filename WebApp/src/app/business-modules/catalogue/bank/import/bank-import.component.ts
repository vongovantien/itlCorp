...
export class BankImportComponent extends AppPage implements OnInit {
    ...
    chooseFile(file: Event) {
        ...
        this._catalogueRepo.upLoadBankFile(file.target['files'])
            .pipe(
                finalize(() => {
    ...
    import(element) {
        ...
        if (this.totalRows - this.totalValidRows > 0) {
            ...
            this._progressRef.start();
            this._catalogueRepo.importBank(data)
                .pipe(
                    finalize(() => {
                        this._progressRef.complete();
                    })
    ...
    downloadSample() {
        this._catalogueRepo.downloadBankExcel()
