...
export class ContainerListComponent extends PopupBase implements OnInit {
      ...
      .pipe(
        ...
        finalize(() => {
          ...
          if (this.listUnits != null) {
            ...
            console.log(this.listWeightMesurements);
            console.log(this.listContainerType);
            console.log(this.listPackageTypes);
          }
        })
      )
      .subscribe(
  ...
  async getComodities() {
    const criteriaSearchCommodity = { inactive: null, all: null };
    const responses = await this.baseServices.postAsync(this.api_menu.Catalogue.Commodity.query, criteriaSearchCommodity, false, false);
    if (responses != null) {
