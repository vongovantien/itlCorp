...
@Component({
    selector: 'import-voucher-advance-payment',
    templateUrl: 'import-voucher-advance-payment.component.html'
})
...
export class ImportVoucherAdvancePaymentComponent extends AppPage implements OnInit {
    ...
    chooseFile(file: Event) {
        ...
        this._accoutingRepo.upLoadVoucherAdvanceFile(file.target['files'])
            .pipe(
                finalize(() => {
    ...
    import(element) {
        ...
        if (this.totalRows - this.totalValidRows > 0) {
            ...
            this._progressRef.start();
            this._accoutingRepo.importVoucherAdvance(data)
                .pipe(
                    finalize(() => {
                        this._progressRef.complete();
                    })
    ...
    downloadSample() {
        this._accoutingRepo.downloadVoucherAdvanceFile()
