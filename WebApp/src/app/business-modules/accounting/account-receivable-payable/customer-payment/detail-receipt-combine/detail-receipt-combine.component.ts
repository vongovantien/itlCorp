      ...
      .subscribe(
        (res: any) => {
          if (!!res) {
            ...
            // check isAllDone and isBalance
            this.CreateReceiptCombineComponent.isAllDone = res.filter(x => x?.syncStatus !== 'Synced' && x.subArcbno === res[0].subArcbno).length > 0 &&
                                                          res.filter(x => x?.syncStatus !== 'Synced' && x.subArcbno === res[0].subArcbno).every(x => x.status === 'Done');
            this.CreateReceiptCombineComponent.isBalance = res.filter(x => x.status === 'Done' && x.subArcbno === res[0].subArcbno).length > 0 &&
                                                          res.filter(x => x.status === 'Done' && x.subArcbno === res[0].subArcbno)?.every(x => !!x.isBalanceReceipt);
                ...
                if (!!creditList.length) {
                  ...
                  creditResult.forEach((value, index) => {
                    ...
                    let itemGrps: any = {
                      ...
                      paymentMethod: !currentDebit.paymentMethod ? this.CreditPaymentReceiptCDCombineComponent.paymentMethodsCredit[0].value : currentDebit.paymentMethod,
                ...
                if (!!debitList.length) {
                  ...
                  debitResult.forEach((value, index) => {
                    ...
                    let itemGrps: any = {
                      ...
                      officeName: currentDebit.officeName,
                      partnerId: currentDebit.customerId,
                      paymentMethod: !currentDebit.paymentMethod ? this.CreditPaymentReceiptCDCombineComponent.paymentMethodsDebit[0].value : currentDebit.paymentMethod,
                      receiptNo: value.paymentRefNo,
                      description: currentDebit.description,
                      status: currentDebit.status,
                      syncStatus: currentDebit.syncStatus,
