    ...
    public class AccountingReportController : ControllerBase
    {
        ...
        public async Task<IActionResult> ExportDetailSettlementPayment(Guid settlementId, string lang, string action)
        {
            ...
            else
            {
                ...
                await _busControl.SendAsync(RabbitExchange.EFMS_ReportData, RabbitConstants.GenFileSyncQueue, model);
        ...
        public async Task<IActionResult> ExportDebitDetail(DebitDetailCriteria criteria)
        {
            ...
            var responseFromApi = await HttpServiceExtension.PostAPI(criteria, aPis.AccountingAPI + Urls.Accounting.GetDebitDetailUrl, null);

            var dataObjects = responseFromApi.Content.ReadAsAsync<List<DebitDetail>>();
            if (dataObjects.Result == null || dataObjects.Result.Count == 0) return Ok();

            //var stream = new AccountingHelper().GenerateAccountingReceivableExcel(dataObjects.Result,criteria.ArType);
            var stream = new AccountingHelper().GenerateAccountingReceivableDebitDetail(dataObjects.Result, criteria.PaymentStatus);
