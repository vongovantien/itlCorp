            ...
            foreach (var acc in accountPayables)
            {
                ...
                foreach (var detail in acc.PaymentDetail)
                {
                    ...
                    if (detail.TransactionType == ForPartnerConstants.PAYABLE_TRANSACTION_TYPE_CREDIT)
                    {
                        ...
                        accPayablePayment.OfficeId = currentUser.OfficeID;
                    ...
                    else if (detail.TransactionType == ForPartnerConstants.PAYABLE_TRANSACTION_TYPE_COMBINE)
                    {
                        ...
                        var payableAdvExisted = DataContext.Get(x => x.TransactionType == ForPartnerConstants.PAYABLE_TRANSACTION_TYPE_ADV && x.ReferenceNo == detail.BravoRefNo && x.OfficeId == office.Id).FirstOrDefault();
                        ...
                        creditPayment.OfficeId = currentUser.OfficeID;
                ...
                try
                {
                    ...
                    foreach (var acc in accountPayables)
                    {
                        ...
                        foreach (var pm in existPayment)
                        {
                            ...
                            hs = DataContext.Update(pm, x => x.Id == pm.Id, false);
                        }
                        var deletePayable = paymentRepository.Get(x => x.PaymentNo == acc.PaymentNo && x.OfficeId == office.Id).Select(x => x.Id).ToList();
                        hsDelPayable = paymentRepository.Delete(x => deletePayable.Any(z => z == x.Id), false);
                    }
