...
namespace eFMS.API.Report.DL.Services
{
    public class CurrencyExchangeService : RepositoryBase<CatCurrencyExchange, CatCurrencyExchangeModel>, ICurrencyExchangeService
    {
        ...
        public CurrencyExchangeService(IContextBase<CatCurrencyExchange> repository, 
            IMapper mapper, IContextBase<CsShipmentSurcharge> surchargeRepo) : base(repository, mapper)
        {
        ...
        public decimal CalculatorAmount(decimal? amount, decimal? finalExchangeRate, DateTime? exchangeDate, string currencyFrom, string currencyTo, int? roundCurr)
        {
            ...
            if (currencyTo == ReportConstants.CURRENCY_LOCAL)
            {
                roundCurr = 0;
            }
            ...
            decimal _exchangeRateCurrencyFrom = GetRateCurrencyExchange(currencyExchange, currencyFrom, ReportConstants.CURRENCY_LOCAL); //Lấy currency Local làm gốc để quy đỗi
            decimal _exchangeRateCurrencyTo = GetRateCurrencyExchange(currencyExchange, currencyTo, ReportConstants.CURRENCY_LOCAL); //Lấy currency Local làm gốc để quy đỗi
        ...
        public AmountResult CalculatorAmountAccountingByCurrency(CsShipmentSurcharge surcharge, string currencyConvert, decimal kickBackExcRate)
        {
            ...
            int _roundDecimal = currencyConvert == ReportConstants.CURRENCY_LOCAL ? 0 : 2; //Local round 0, ngoại tệ round 2
            decimal _netAmount = 0;
            decimal _vatAmount = 0;
            decimal _excRate = 0;

            //Tính tỉ giá Final Exchange Rate (Tỉ giá so với LOCAL)
            decimal exchangeRateToLocal = (surcharge.Type == ReportConstants.CHARGE_BUY_TYPE && surcharge.KickBack == true && surcharge.CurrencyId == ReportConstants.CURRENCY_USD) ? kickBackExcRate : CurrencyExchangeRateConvert(surcharge.FinalExchangeRate, surcharge.ExchangeDate, surcharge.CurrencyId, ReportConstants.CURRENCY_LOCAL);
            ...
            else
            {
                ...
                if (surcharge.Type == ReportConstants.CHARGE_BUY_TYPE && surcharge.KickBack == true)
                {
                    if (surcharge.CurrencyId == ReportConstants.CURRENCY_USD && currencyConvert == ReportConstants.CURRENCY_LOCAL)
                    {
                        exchangeRate = kickBackExcRate;
                    }
                    if (surcharge.CurrencyId == ReportConstants.CURRENCY_LOCAL && currencyConvert == ReportConstants.CURRENCY_USD)
                    {

                        //var _exChangeRateUsd = CurrencyExchangeRateConvert(surcharge.FinalExchangeRate, surcharge.ExchangeDate, ReportConstants.CURRENCY_LOCAL, ReportConstants.CURRENCY_USD);
                ...
                if (surcharge.Vatrate != null)
                {
                    var vatAmount = surcharge.Vatrate < 0 ? (Math.Abs(surcharge.Vatrate ?? 0) * exchangeRate) : ((_netAmount * surcharge.Vatrate) ?? 0) / 100;
                    _vatAmount = NumberHelper.RoundNumber(vatAmount, _roundDecimal);
                }
            }
        ...
        public AmountSurchargeResult CalculatorAmountSurcharge(CsShipmentSurcharge surcharge, decimal kickBackExcRate)
        {
            ...
            if (surcharge.CurrencyId == ReportConstants.CURRENCY_LOCAL)
            {
            ...
            {
                var amountLocal = CalculatorAmountAccountingByCurrency(surcharge, ReportConstants.CURRENCY_LOCAL, kickBackExcRate);
            ...
            if (surcharge.CurrencyId == ReportConstants.CURRENCY_USD)
            {
            ...
            {
                var amountUsd = CalculatorAmountAccountingByCurrency(surcharge, ReportConstants.CURRENCY_USD, kickBackExcRate);
        ...
        public decimal ConvertAmountChargeToAmountObj(CsShipmentSurcharge surcharge, string currencyObject)
        {
            ...
            if (currencyObject == ReportConstants.CURRENCY_LOCAL)
            {
                _totalAmount = (surcharge.AmountVnd + surcharge.VatAmountVnd) ?? 0;
            }
            else if (currencyObject == ReportConstants.CURRENCY_USD)
            {
        ...
        public decimal CurrencyExchangeRateConvert(decimal? finalExchangeRate, DateTime? exchangeDate, string currencyFrom, string currencyTo)
        {
            ...
            if (finalExchangeRate != null)
            {
                if (currencyFrom != ReportConstants.CURRENCY_LOCAL && currencyTo == ReportConstants.CURRENCY_LOCAL)
                {
            ...
            if (finalExchangeRate != null)
            {
                ...
                //else if (currencyFrom != ReportConstants.CURRENCY_LOCAL && currencyTo == ReportConstants.CURRENCY_LOCAL)
                //{
            ...
            else
            {
                ...
                {
                    _exchangeRate = GetRateCurrencyExchange(currencyExchange, currencyFrom, ReportConstants.CURRENCY_LOCAL);
                }
                else
                {
