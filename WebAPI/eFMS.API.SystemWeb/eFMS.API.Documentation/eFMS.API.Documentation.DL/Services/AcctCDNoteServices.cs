...
namespace eFMS.API.Documentation.DL.Services
{
    public class AcctCDNoteServices : RepositoryBase<AcctCdnote, AcctCdnoteModel>, IAcctCDNoteServices
    {
        ...
        private IQueryable<InvoiceListModel> GetDataSoaNotIssuedCdNote(CDNoteCriteria criteria)
        {
            ...
            {
                IEnumerable<string> refNos = criteria.ReferenceNos.Split('\n').Select(x => x.Trim()).Where(x => x != null);
                var surchargesCdNote = charges.Where(x => refNos.Any(a => a == x.JobNo || a == x.Mblno || a == x.Hblno || a == x.Soano) && !string.IsNullOrEmpty(x.Soano) && string.IsNullOrEmpty(x.CreditNo) && string.IsNullOrEmpty(x.DebitNo)).Select(s => s.Soano).ToList();
                surchargesCdNote.AddRange(charges.Where(x => refNos.Any(a => a == x.JobNo || a == x.Mblno || a == x.Hblno || a == x.Soano) && !string.IsNullOrEmpty(x.PaySoano) && string.IsNullOrEmpty(x.CreditNo) && string.IsNullOrEmpty(x.DebitNo)).Select(s => s.PaySoano).ToList());
                if (surchargesCdNote.Count > 0)
                {
        ...
        private IQueryable<InvoiceListModel> GetDataCdNote(CDNoteCriteria criteria)
        {
            ...
            if (!string.IsNullOrEmpty(criteria.ReferenceNos))
            {
                ...
                surchargesCdNote.AddRange(charges.Where(x => refNos.Any(a => a == x.JobNo || a == x.Mblno || a == x.Hblno || a == x.Soano) && !string.IsNullOrEmpty(x.CreditNo)).Select(s => s.CreditNo).ToList());
                if (surchargesCdNote.Count > 0)
                {
                    query = query.And(x => refNos.Any(a => a == x.Code) || surchargesCdNote.Any(a => a == x.Code));
                }
        ...
        private IQueryable<InvoiceListModel> GetDataCdNoteAgency(CDNoteCriteria criteria)
        {
            ...
            var creditData = from cd in cdNoteData
                             ...
                             sc.Type!="OBH" && (String.IsNullOrEmpty(sc.SyncedFrom) || sc.SyncedFrom != "SOA")
                             select new InvoiceListModel
                             {
                                 JobNo = sc.JobNo,
            ...
            // HashSet<string> creditSoaNos = new HashSet<string>(creditData.Select(z => z.SoaNo));

            // var newSoaDat = from p in soadat
               //             where !debitSoaNos.Contains(p.SoaNo) || !creditSoaNos.Contains(p.SoaNo)
                //            select p;
